...
:::caution[`List<T>` Non è Thread-Safe]
...
- Lo stesso thread che aggiunge elementi è anche quello che li rimuove (pattern **thread-local**)
...
- Eccellente scalabilità con molti thread
...
Per scenari Producer-Consumer dove il consumer deve attendere elementi, `BlockingCollection<T>` (che internamente può usare `ConcurrentQueue<T>`) offre metodi bloccanti come `Take()` e `Add()`, eliminando la necessità di polling.
...
Queste operazioni sono più efficienti di loop multipli.
:::
...
- Predominanza di letture (99%+) → Considera `ImmutableDictionary` o cache read-only
- Single-threaded → Dictionary normale è 2-3x più veloce
- Richiesto ordine di iterazione → usa `SortedDictionary` + lock
...
| --------------------------- | ---------------------------- | ------------------- |
| Complessità implementazione | Alta (gestione manuale)      | Bassa (built-in)    |
...
In questi casi, le collezioni standard sono 2-5x più veloci.
:::
