...
##### PUT - Aggiornare completamente una risorsa
...
[REST Client](https://github.com/Huachao/vscode-restclient) è un'estensione per Visual Studio Code che permette di inviare richieste HTTP direttamente dall'editor, senza bisogno di strumenti esterni come Postman o cURL. Le richieste vengono scritte in file con estensione `.http` o `.rest` e possono essere eseguite con un semplice click.
...
:::note
Il separatore `###` permette di dividere più richieste nello stesso file. Ogni richiesta può essere eseguita singolarmente posizionando il cursore su di essa.
:::

:::tip[Esecuzione]
Posizionare il cursore sulla riga della richiesta desiderata (ad esempio `GET https://...`) e premere `Ctrl+Alt+R` per eseguirla.
:::

### Generazione di comandi cURL
...
:::note
Questa funzionalità permette di prototipare rapidamente le richieste in REST Client e poi esportarle per l'uso in script shell o documentazione.
:::
...
- Repository GitHub: [https://github.com/Huachao/vscode-restclient](https://github.com/Huachao/vscode-restclient)
- Marketplace VS Code: Cercare "REST Client" nell'apposita sezione

## Utilizzo di un Mock Server per il testing di applicazioni client <Badge text="Importante" variant="tip" size="medium" />
...
Per facilitare l'apprendimento e il testing delle applicazioni client, viene fornita una cartella preconfigurata contenente tutti i file necessari per avviare un mock server REST. La cartella è scaricabile dal repository degli esempi: [mock-server-tutorial](https://github.com/malafronte/malafronte-doc-samples/tree/main/samples-quarta/api_client_server_demos/mock-server-tutorial). Per scaricare solo la cartella specifica, è possibile utilizzare un applicativo web come [download-directory.github.io](https://download-directory.github.io/) oppure il comando `git sparse-checkout`per clonare solo la parte desiderata del repository, oppure utilizzare uno degli script ottimizzati indicati nella terza opzione di seguito.
...
1. Visitare [https://download-directory.github.io/](https://download-directory.github.io/)
...
:::note[Nota su GitHub]
Il pulsante "Download ZIP" nativo di GitHub scarica sempre l'intero repository, non le singole sottocartelle. Per scaricare solo una cartella specifica senza Git, è necessario utilizzare strumenti di terze parti come download-directory.github.io o i comandi sparse-checkout mostrati sopra.
:::
...
:::note[Spiegazione dei Comandi Git Sparse-Checkout]

Questi comandi permettono di scaricare **solo una specifica cartella** da un repository Git invece di clonare l'intero repository, risparmiando tempo e spazio su disco.

**1. `git clone --filter=blob:none --no-checkout <URL>`**

- `--filter=blob:none`: Utilizza il "partial clone" per scaricare solo i metadati del repository (commit history, tree structure) senza scaricare i contenuti dei file (blobs). Questo riduce drasticamente il download iniziale.
- `--no-checkout`: Evita di creare la copia di lavoro (working directory) con i file, poiché scaricheremo solo una parte selezionata.

**2. `cd malafronte-doc-samples`**

- Entra nella directory del repository appena clonato.

**3. `git sparse-checkout init --cone`**

- Inizializza la funzionalità **sparse-checkout**.
- `--cone`: Abilita la modalità "cone pattern matching", che è più efficiente e permette di definire cartelle "radice" (cone roots). Le cartelle specificate verranno scaricate con tutti i loro contenuti.

**4. `git sparse-checkout set <percorso-cartella>`**

- Specifica quale cartella scaricare. In questo caso: `samples-quarta/api_client_server_demos/mock-server-tutorial`
- Git scaricherà solo i file contenuti in questa cartella e nelle sue sottocartelle.
- Tutte le altre cartelle del repository rimarranno "vuote" (o non verranno scaricate).

**5. `git checkout`**

- Applica le impostazioni sparse-checkout al working directory.
- Scarica effettivamente i file della cartella specificata nel comando precedente.

:::note[Vantaggi dello Sparse-Checkout]

- **Risparmio di spazio**: Scarica solo i file necessari invece dell'intero repository (potenzialmente centinaia di MB).
- **Download più veloce**: Meno dati da trasferire tramite rete.
- **Organizzazione pulita**: La directory locale contiene solo i file pertinenti al progetto.
  :::

#### Opzione 3: Utilizzare script ottimizzati per scaricare solo la cartella specifica

Se si preferisce un approccio ancora semplice e sicuro, è possibile utilizzare uno script che automatizza il download della cartella specifica. Nella cartella [tools-and-scripts](https://github.com/malafronte/malafronte-doc-samples/tree/main/tools-and-scripts/github-download-directory) del repository [https://github.com/malafronte/malafronte-doc-samples](https://github.com/malafronte/malafronte-doc-samples) sono riportati alcuni script sia bash che python che permettono di scaricare solo una cartella specifica da un repository GitHub.

- [download-github-folder-optimized.py](https://github.com/malafronte/malafronte-doc-samples/blob/main/tools-and-scripts/github-download-directory/download-github-folder-optimized.py): Script Python ottimizzato per scaricare solo la cartella desiderata, con gestione dell'autenticazione e degli errori.
- [download-github-folder-optimized.sh](https://github.com/malafronte/malafronte-doc-samples/blob/main/tools-and-scripts/github-download-directory/download-github-folder-optimized.sh): Script bash ottimizzato per scaricare solo la cartella desiderata, con gestione dell'autenticazione e degli errori, ideale per ambienti Unix/Linux o Windows con WSL.

Per maggiori dettagli si può consultare la pagina [README](https://github.com/malafronte/malafronte-doc-samples/blob/main/tools-and-scripts/github-download-directory/README.md) della cartella.
...
##### Come rimuovere il proxy di npm
...
## Un esempio completo di client C# con backend mock JSON-Server e supporto all'autenticazione mediante token <Badge text="Importante" variant="tip" size="medium" />
...
1. Nella prima fase, il progetto [**mock-server-products**](https://github.com/malafronte/malafronte-doc-samples/tree/main/samples-quarta/api_client_server_demos/products/mock-server-products) implementa un server mock basato su json-server che espone un'API RESTful completa per la gestione di prodotti. Questo server supporta tutte le operazioni CRUD (GET, POST, PUT, PATCH, DELETE) sulla risorsa `/products`, utilizza dati iniziali provenienti da un file JSON e include risorse multimediali locali (immagini, miniature e QR code) servite dalla cartella `public/assets/products_media`. Il server si avvia sulla porta 3000 e offre anche la possibilità di generare dataset casuali tramite `Faker.js`. L'applicazione [**ApiClient**](https://github.com/malafronte/malafronte-doc-samples/tree/main/samples-quarta/api_client_server_demos/products/ApiClient) rappresenta il client C# che interagisce con questo server, dimostrando come effettuare richieste HTTP e gestire le risposte RESTful in un ambiente di testing controllato. 
3. Passando alla seconda fase, il progetto [**mock-server-auth**](https://github.com/malafronte/malafronte-doc-samples/tree/main/samples-quarta/api_client_server_demos/products/mock-server-auth) estende il server mock introducendo un sistema di autenticazione basato su JWT (JSON Web Token). In questa configurazione, le operazioni di lettura (GET) rimangono pubblicamente accessibili, mentre tutte le operazioni di modifica (POST, PUT, PATCH, DELETE) richiedono un token JWT valido nell'header `Authorization`. Il server espone un endpoint `/login` che, ricevute credenziali valide (email e password), restituisce un token JWT da utilizzare nelle richieste successive. Il client [**ApiClientAuth**](https://github.com/malafronte/malafronte-doc-samples/tree/main/samples-quarta/api_client_server_demos/products/ApiClientAuth) implementa la logica necessaria per gestire questo flusso di autenticazione: effettua il login inviando le credenziali, estrae e memorizza il token JWT ricevuto, configura l'istanza di `HttpClient` per includere automaticamente il token nell'header `Authorization` (con schema Bearer) e procede con l'esecuzione delle operazioni CRUD sugli endpoint protetti. Questa progressione permette di comprendere prima le basi della comunicazione HTTP con un mock server, per poi affrontare gli aspetti di sicurezza tipici delle API RESTful professionali, dove l'autenticazione tramite token rappresenta lo standard de facto per proteggere le risorse sensibili.

## Creazione di Mock Server REST con `json-server` e `faker.js` <Badge text="Approfondimento" variant="caution" size="medium" />
...
## Postman Mock Server <Badge text="Approfondimento" variant="caution" size="medium" />
