...
Il problema è dovuto al fatto che cURL non riesce a verificare l'autenticità del certificato digitale presentato dal server remoto. Per risolvere questo problema è necessario configurare cURL in modo che possa accedere ai certificati delle Certification Authorities (CA) che consentono di validare il certificato digitale del server, seguendo il meccanismo di verifica basato su crittografia asimmetrica e firma digitale.
...
- Il browser contiene preinstallate le chiavi pubbliche delle **Root CA** (autorità di certificazione radice), organizzazioni altamente affidabili come Let's Encrypt, DigiCert, GlobalSign
- Queste Root CA possono firmare certificati per server web, oppure firmare certificati di **Intermediate CA** (autorità intermedie)
- Le Intermediate CA a loro volta possono firmare certificati per i server

Il browser verifica l'intera catena: il certificato del server è firmato da un'Intermediate CA, il cui certificato è firmato da una Root CA. Se l'intera catena è valida, il certificato del server è considerato **trusted** (fidato).
