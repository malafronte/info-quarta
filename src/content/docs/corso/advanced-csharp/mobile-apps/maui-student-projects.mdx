...
In tutte le guide passo-passo che seguono, si intende **sempre** applicato il seguente rigido loop di lavoro con Git:
1. **Plan & Branch**: all'inizio del passo, richiedere la creazione di un branch specifico (es. `git checkout -b feature/ui-meteo-attuale`).
2. **Execute & Test**: sviluppare e testare la feature (con o senza l'ausilio dell'Intelligenza Artificiale).
3. **Commit & Merge**: alla fine del passo, formattare il commit (es. `git commit -am "feat: completata UI meteo attuale"`) per poi eventualmente chiudere il branch. L'agente AI può essere istruito ad eseguire queste automazioni tramite terminale a patto che gli si concedano i dovuti permessi controllati (vedi l'Esempio Completo in fondo a questo documento).
