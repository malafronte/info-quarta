...
### 6.4 Fase 4 - Uso di GitHub Copilot, KiloCode, OpenCode, Claude Code, etc.  durante lo sviluppo

GitHub Copilot, oppure KiloCode, OpenCode, Claude Code, etc., non vanno usati solo come completamento automatico di singole righe.  
Possono essere impiegati anche per pianificare, spiegare, ri-fattorizzare e verificare modifiche.
...
#### Esempio pratico: creare una skill di code reviews per scoprire skill utili per .NET MAUI
...
2. **`github/awesome-copilot@breakdown-feature-prd`** (7.8K installazioni) — per spezzare il PRD in feature individuali (perfetto per le iterazioni del workflow Man-in-the-Loop descritto nella sezione 3!)
