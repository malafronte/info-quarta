...
graph TD
    ...
    Test -- "✅ Pass" --> Doc["<b>5. Documentazione e Git</b><br/>Aggiornare log,<br/>eseguire Commit e Push"]
...
- **creare un nuovo branch Git** dedicato all'iterazione o alla feature (es. `git checkout -b feature/book-detail` oppure `git checkout -b it-03-dettaglio`).
...
Al termine di ogni iterazione si aggiornano i file di documentazione e si esegue il consolidamento tramite strumento di versioning (Git):
...
- si verifica lo stato dei file modificati con `git status` e si analizzano le differenze con `git diff`;
- si esegue il commit di tutte le modifiche (o si delega all'AI la stesura di un commit message convenzionale) e si spinge il branch sul repository remoto (es. `git commit -am "feat: implementata pagina di dettaglio"`, seguito da `git push`).

Questa fase chiude il ciclo dell'iterazione e la consolida tracciando la storicità ed i passaggi di sviluppo, preparando il terreno per l'iterazione successiva e l'eventuale "merge" con il branch principale.
...
[OpenCode](https://opencode.ai/) è un framework agente che lavora da riga di comando o integrato in un editor.  
...
### 5.3 Modelli Cloud e Locali (Ollama, KiloCode, OpenCode Zen)
...
[Ollama](https://ollama.com/) è un framework per eseguire modelli linguistici in locale sulla propria macchina di sviluppo.

Vantaggi:

- il codice non lascia la macchina locale (privacy totale);
- non ci sono limiti di rate o di quota;
- possibilità di scegliere il modello più adatto (Llama3, DeepSeek, Qwen3.5, ecc.).
...
**[ Ollama Cloud](https://docs.ollama.com/cloud)** è l'infrastruttura ufficiale ospitata in rete dai creatori di Ollama. 
...
**OpenRouter** ([openrouter.ai/docs/quickstart](https://openrouter.ai/docs/quickstart)) è in ambito storiografico l'AI Gateway e l'aggregatore di modelli di intelligenza artificiale di gran lunga più rinomato. 
...
[Google Antigravity](https://antigravity.google/docs/get-started) rappresenta l'evoluzione dell'IDE tradizionale verso la nuova era *"Agent-First"*. Sviluppata da Google DeepMind, non è un semplice strumento di autocompletamento, ma una vera e propria **piattaforma di sviluppo agentica** pensata per dirigere e orchestrare AI autonome. 
...
### 5.4 L'agente AI e le operazioni Git (Commit e Push)
...
- **Il potenziale dell'automazione**: Chiedere semplicemente *"Crea il branch it-04-persistenza e committa le modifiche che abbiamo appena testato con un messaggio convenzionale"* fa risparmiare tempo e migliora vertiginosamente la qualità del log di Git, che terrà esattamente traccia di ogni variazione del software.
...
ProjectRoot/
├─ .git/                        # Folder di Git
├─ .gitignore                   # Esclusioni per il version control
...
<Accordion title="Esempio di Skill per lo sviluppo MAUI">
